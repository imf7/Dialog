...
if(flag==0){parent.setAttribute("data-type","dialog");parent.setAttribute("data-dialog-name",this.name);head.className="D_head";content.className="D_content";content.id=this.config_id;if(this.config_closeButton){head_html+='<a href="#" class="D_close" title="\u5173\u95ed" data-dialog-close>\u00d7</a>';}
...
return this;};Dialog.prototype.animation=function(){var that=this;if(this.config_animation){this.content.className+=" "+this.config_animation;var events=["animationend","webkitAnimationEnd","mozAnimationEnd","MSAnimationEnd","oanimationend"];for(var i=0;i<events.length;i++){this.content.addEventListener(events[i],function(){addFlash(arguments.callee);});}
...
Dialog.controller[name].close();}};function escCloseDialog(){document.onkeydown=function(e){var childs=document.getElementsByTagName("body")[0].childNodes,dialog=[],elem;for(var i=0,l=childs.length;i<l;i++){if(childs[i].nodeName=="DIV"&&childs[i].getAttribute("data-type")=="dialog"){dialog.push(childs[i]);}}
